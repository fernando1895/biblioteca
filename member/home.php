...
<html>
	...
	<body>
		<?php
			...
			if($rows == 0)
				echo "<h2 align='center'>No se encuentran libros</h2>";
			else
			{
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<legend h2 legend align='center'>Libros disponibles</legend>";
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					echo "<tr h2 legend align='center'>
							<td>
			...
			if(isset($_POST['m_request']))
			{
				...
				{
					$query = $con->prepare("SELECT prestamo FROM book WHERE prestamo = ?;");
					$query->bind_param("s", $_POST['rd_book']);
					$query->execute();
					$prestamo = mysqli_fetch_array($query->get_result())[0];
					if($presatamo == 0)
						echo error_without_field("No es posible prestar el libro fuera de la biblioteca.");
					else
				    {
